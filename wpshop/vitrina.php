<script>
$(function(){
	$('.vitrina_wrap .partnerka').each(function() {
		var part = $(this).find('.partnerka_url').text();
			$(this).find('.wpshop_bag .wpshop_buy .line_1 td.wpshop_button').html('<a href="'+part+'" target="_blank"><span><?php _e( '_', 'wp-shop' ); ?></span></a>');
	});
});
</script>		
...
while($query->have_posts())
{
	...
	$part_url = get_post_meta($query->post->ID, 'part_url', true);
	...
	if ($part_url !=''){echo "class='partnerka' ";}
	echo ">";
	if ($part_url !=''){?>
