...
<div class="container relat clearfix">
	...
	<div id="main-content" class="narrow <?php if (of_get_option('blog_sidebar_pos')=='right') {echo ' floatLeft';} else {echo 'floatRight';} ?>">
		<?php if ( have_posts() ) : ?>
		<h1 class="search_title"><?php printf( __( 'Вы искали: %s', 'wp-shop' ), '<span>"' . get_search_query() . '"</span>' ); ?></h1>
      ...
      <input type="hidden" name="s" value="<?php echo $_GET['s'];?>">
		...
		<?php if (function_exists('wp_corenavi')) wp_corenavi(); ?>
		<?php else : ?>
		<h1 class="search_title"><?php _e( 'Извините по Вашему запросу ничего не найдено', 'wp-shop' ); ?></h1>
		<div class="clear"></div>
